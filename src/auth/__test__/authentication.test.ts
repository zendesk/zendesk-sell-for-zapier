...
describe('OAuth2 flow', () => {
  ...
  it('Step 1 - generate an authorize url', async () => {
    ...
    const authenticationUrl = await appTester(App.authentication.oauth2Config.authorizeUrl, bundle)
    expect(authenticationUrl).toEqual('https://api.getbase.com/oauth2/authorize?client_id=1234566890&state=1234&redirect_uri=http%3A%2F%2Fzapier.com&response_type=code')
  })
  ...
  it('Step 2 - fetches access token', async () => {
    ...
    const response = await appTester(App.authentication.oauth2Config.getAccessToken, bundle)
  ...
  it('Step 2 - throws exception when different code than 200 is returned', async (done) => {
    const bundle = {
      inputData: {
        code: 'one-time-code'
      }
    ...
    try {
      ...
      done.fail('401 API response should throw exception')
    } catch (e) {
  ...
  it('Step 3 - refreshes access token', async () => {
    ...
    nock('https://api.getbase.com', {reqheaders: authorizationBase64Header})
      .post('/oauth2/token', 'refresh_token=old_refresh_token&grant_type=refresh_token')
      .reply(200, refreshToken)

    const response = await appTester(App.authentication.oauth2Config.refreshAccessToken, bundle)
    expect(response).toEqual({
