 ...
 */
export const searchWithPrefixedFields = (endpoint: string, actionDetails: ActionDetails, supportedFilters: string[]) => {
  return async (z: ZObject, bundle: Bundle, otherFilters: Filters = {}) => {
...
/**
 * It allows to fetch more than one page from the API. Use this one for triggers, this is the only use case (so far) when we
 * want to load more data than one page contains (> 100).
 *
 * Optimization: If trigger is called from web editor, only first page is fetched (it doesn't make sense to fetch more than one in this scenario)
 */
export const streamItems = (endpoint: string, actionDetails: ActionDetails, supportedFilters: string[], maxPages: number = 5) => {
  return async (z: ZObject, bundle: Bundle, otherFilters: Filters = {}, sort?: SortDefinition) => {
    const payload = {
      url: endpoint,
      params: unpackSupportedParams(supportedFilters)(bundle, otherFilters, sort)
...
const streamPages = async (z: ZObject, options: FetchRequestOptions, actionDetails: ActionDetails, maxPage: number): Promise<any[]> => {
  const stream = async (currentPage: number, shouldContinue: boolean, itemsAcc: any[]): Promise<any[]> => {
    ...
    return await stream(currentPage + 1, hasResponseNextPage(response), [...itemsAcc, ...response.items])
  }
...
const fetchNextPage = async (z: ZObject, options: FetchRequestOptions, actionDetails: ActionDetails, pageNumber: number) => {
  const response = await fetch(
    z,
    {
      ...options,
