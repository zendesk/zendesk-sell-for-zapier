...
import {CreateLeadAction, DeprecatedCreateOrUpdateLeadAction, UpdateLeadAction} from '../lead.action'
...
describe('create lead action', () => {
  it('should pass if lead is properly created', async () => {
    ...
    const response = await appTester(App.creates[CreateLeadAction.key].operation.perform, bundle)
    expect(response.id).toEqual(100)
  })

  it('should throw error with details if 4xx is returned from API while creating lead', async (done) => {
    const bundle = {
      authData: {
        api_token: 'api token'
      },
    ...
    try {
      await appTester(App.creates[CreateLeadAction.key].operation.perform, bundle)
  ...
  it('should throw error if validation fails', async (done) => {
    const bundle = {
      authData: {
        api_token: 'api token'
      },
    ...
    try {
      await appTester(App.creates[CreateLeadAction.key].operation.perform, bundle)
      done.fail('Incorrect payload should throw exception')
    } catch (e) {
      expect(e.message).toContain('Make sure a lead has at least Last Name or Company Name provided.')
      done()
    }
  })
})
...
describe('update lead action', () => {
  it('should pass if lead is properly updated', async () => {
    ...
    const response = await appTester(App.creates[UpdateLeadAction.key].operation.perform, bundle)
    expect(response.id).toEqual(100)
  })
})
...
describe('create or update lead action', () => {
  ...
  it('should create new lead if one with provided id doesn\'t exist', async () => {
    ...
    const response = await appTester(App.creates[DeprecatedCreateOrUpdateLeadAction.key].operation.perform, bundle)
