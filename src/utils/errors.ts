import { BaseHttpResponse, HttpResponse, ZObject } from 'zapier-platform-core'
import {
  isForbidden,
  isNotFound,
  isRateLimitReached,
...
const errorResponseFromMalformedEnvelope = (
  status: number,
  statusMessage: string,
  details: string
): ErrorResponse => {
...
const parseErrorResponse = (
  ...
  if (isRateLimitReached(status)) {
    return errorResponseFromMalformedEnvelope(
      status,
      '429 Too Many Requests',
      'You reached rate limit for Sell API'
    )
  }
  try {
...
const extractErrorDetails = (response: ErrorResponse): string[] =>
  response.errors.map((error) => error.error.details)

const extractStatusMessage = (response: ErrorResponse): string =>
  response.meta.http_status
...
export const extractErrorMessageFromEnvelope = (
  z: ZObject,
  response: HttpResponse
): ErrorDetails => {
 ...
 */
export const formatAndThrowAPIError = (
  z: ZObject,
  response: HttpResponse
): never => {
  const { statusCode, statusMessage, errorDetails, query, url } =
    extractErrorMessageFromEnvelope(z, response)
